/* --------------------------------------------------------------
    ...
    Copyright 1999-2024, Р.Н.Шакиров, ИМаш УрО PAH.
    All Rights Reserved.
...
{
  cBigTemp a; swap (a);
  size_t n = a.length() + 1, m = mod.length() * 2 + 2;
#ifdef  _CBIGNUM_SUBMUL_DIV
  n += 1;
#endif//_CBIGNUM_SUBMUL_DIV
  checkexpand (m); a.checkexpand (size_max (n, m));
...
cBigNumber& cBigNumber::powmod (const cBigNumber& b, CBNL mod)
{
  ...
  n += 1;
#endif//_CBIGNUM_SUBMUL_DIV
  checkexpand (4); a.checkexpand (size_max (n, 4));
  cBigNumberPowMod (EXPTRTYPE(a), CBPTRTYPE(b),
...
#ifdef  _CBIGNUM_SUBMUL_DIV                     // и остатка в mod_.
