...
int    _CBNL_C _cBigNumberCompHigh
                          (const CBPTR(CBNL) p1, const CBNL* p2, size_t k2);
...
void   _cBigNumberTab           (EXPTR(CBNL) p1, CBNC ns);
...
size_t _CBNL_C _cBigNumberMSubD (EXPTR(CBNL) p1, const CBPTR(CBNL) p2,
                                 size_t k2);
#else //_CBNL_TAB_FULL
size_t _CBNL_C _cBigNumberMSubD (EXPTR(CBNL) p1, const CBNL* p2,
                                 size_t k2);
...
void    cBigNumberMModDivShlTab2(EXPTR(CBNL) p1, const CBPTR(CBNL) p2,
                                 size_t k2, EXPTR(CBNL) p);
void    cBigNumberMModDiv       (EXPTR(CBNL) p1, const CBPTR(CBNL) p2,
                                 EXPTR(CBNL) p);
void    cBigNumberMModShlTab    (EXPTR(CBNL) p1, const CBPTR(CBNL) p2,
                                 size_t k2);
void    cBigNumberMModShlTab2   (EXPTR(CBNL) p1, const CBPTR(CBNL) p2,
                                 size_t k2);
...
inline  int    cBigNumberCompHigh (const CBPTR(CBNL) p1, const CBPTR(CBNL) p2)
{
  return _cBigNumberCompHigh (p1, CBPTRBASE(p2),
                        (size_t)(*CBPTRBASE(p1) - *CBPTRBASE(p2)));
}
...
inline  void    cBigNumberTab           (EXPTR(CBNL) p1)
{
  _cBigNumberTab (p1, CHAR_BIT * sizeof (CBNL));
}
...
#else //_CBNL_TAB_FULL
inline  size_t  cBigNumberMSubD        (EXPTR(CBNL) p1, const CBNL* p2,
                                        size_t k2)
{
  *(p1) = (CBNL)((size_t)(*p2) + k2);
  return (size_t)(*(p1)=(CBNL)_cBigNumberMSubD (p1, p2, k2));
}
#endif//_CBNL_TAB_FULL
